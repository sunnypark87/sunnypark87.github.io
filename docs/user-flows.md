...
이 문서는 현재 정보 구조에서 검증한 최소 탐색 흐름과 구현 상태를 기록한다. Knowledge Network는 별도 그래프 화면이 아니라 실제 콘텐츠 관계 링크로 먼저 표현한다. 검증용 공개 Article 2개를 기준으로 자동·정적 검사와 모바일·키보드 브라우저 검증을 완료했다.
...
- `content-pipeline-fixture`와 `markdown-content-relations` 두 Article로 관계 탐색을 검증한다.
- `content-pipeline-fixture`는 현재 4단계 흐름 검증을 위한 공개 콘텐츠로 유지하며, 최종 공개 전 실제 글로 교체하거나 내용을 정식화한다.
- Article 하단에 실제 관계가 존재할 때만 `먼저 읽으면 좋은 글`, `관련 글`, `다음 단계` 그룹을 표시한다.
- 관계 대상은 공개 Article만 해석하며, draft·존재하지 않는 대상은 콘텐츠 검증에서 실패한다.
- Project가 없는 상태이므로 Project 상세 흐름은 이월한다.
- 320px/375px/768px/데스크톱 화면, 키보드 Tab 순서, 긴 제목·코드·표·수식의 가독성을 검증했고 발견된 문제는 없었다.
- 상세 검증 기록은 [`user-flow-validation.md`](user-flow-validation.md)에 남긴다.
